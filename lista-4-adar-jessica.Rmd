...
num_dias <- nlayers(grid_prec_raster)  
dates <- seq(inicio_date, by = "day", length.out = num_dias)
  
colnames(grad_prec)[-(1:2)] <- as.character(dates)
  
grad_prec_tidy <- grad_prec %>%
  pivot_longer(
    cols = -c(x, y),
    names_to = "date",
    values_to = "prec" 
